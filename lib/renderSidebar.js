...
function renderLearnLink(email) {
  ...
  // NOT URL-encoded: ThriveCart's Learn search is a client-side hash router
  // that doesn't decode the query string, so an encoded "@" (%40) fails to
  // match -- confirmed live, a raw "@" in the URL is what actually works.
  const href = template
    .replace('{accountSlug}', encodeURIComponent(accountSlug))
    .replace('{email}', email ?? '');
