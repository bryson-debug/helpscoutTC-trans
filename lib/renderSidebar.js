...
function baseStyles() {
  return `
    <style>
      ...
      .tc-ltv { display: flex; justify-content: space-between; align-items: baseline; padding-bottom: 10px; margin-bottom: 4px; border-bottom: 1px solid #eceef1; }
      .tc-ltv-label { font-size: 12px; font-weight: 600; color: #55575c; text-transform: uppercase; letter-spacing: 0.02em; }
      .tc-ltv-amount { display: inline-block; font-size: 13px; font-weight: 700; background: #e3f5e9; color: #1c7a3f; padding: 3px 10px; border-radius: 999px; }
      .tc-subheading { font-size: 12px; font-weight: 600; color: #55575c; text-transform: uppercase; letter-spacing: 0.02em; }
      .tc-group-header { display: flex; align-items: center; justify-content: space-between; width: 100%; background: none; border: none; padding: 0; margin: 12px 0 6px; cursor: pointer; text-align: left; }
      ...
      .tc-btn-row { display: flex; gap: 8px; margin-top: 10px; }
      ...
      .tc-link-btn--secondary { background: #fff; color: #314ddb !important; border: 1px solid #314ddb; }
      .tc-link-btn--secondary:hover { background: #eef0fd; }
...
function renderLearnLink(email) {
  // Confirmed live: ThriveCart Learn has no safe read-only "find student by
  // email" API (its /students endpoint enrolls a student -- a write action
  // with side effects like a signin email, not something to call just to
  // build a link), so this links to the pre-filled student search instead
  // of a direct one-click profile link.
  const accountSlug = process.env.THRIVECART_ACCOUNT_SLUG || 'thatmusicteacher';
  const template =
    process.env.THRIVECART_LEARN_URL_TEMPLATE || 'https://thrivecart.com/{accountSlug}/#/learn/students?search={email}';
  const href = template
    .replace('{accountSlug}', encodeURIComponent(accountSlug))
    .replace('{email}', encodeURIComponent(email ?? ''));
  return `<a class="tc-link-btn tc-link-btn--secondary" href="${escapeHtml(href)}" target="_blank" rel="noopener noreferrer">Learn</a>`;
}
