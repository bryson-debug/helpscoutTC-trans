...
function baseStyles() {
  return `
    <style>
      ...
      .tc-heading { font-size: 14px; font-weight: 600; margin: 16px 0 8px; }
      ...
      .tc-link-btn { display: inline-block; margin-top: 10px; padding: 6px 12px; background: #1292ee; color: #fff !important; text-decoration: none; border-radius: 4px; font-size: 12px; font-weight: 600; }
      .tc-link-btn:hover { background: #0f7ad1; }
...
function renderProfileLink(customerId, email) {
  // ThriveCart's customer API returns no ID field at all (confirmed live),
  // so the link is built from email by default -- confirm the real
  // ThriveCart dashboard search/profile URL pattern and adjust.
  const template = process.env.THRIVECART_CUSTOMER_URL_TEMPLATE || 'https://thrivecart.com/customers?search={email}';
  const href = template
...
function wrap(bodyHtml) {
  return `<div class="tc-sidebar">${baseStyles()}<div class="tc-heading">Transactions</div>${bodyHtml}</div>`;
}
