...
function renderLearnLink(email) {
  ...
  // side. Links to the plain student search screen instead, and copies the
  // email to the clipboard on click so the agent can just paste it in.
  const accountSlug = process.env.THRIVECART_ACCOUNT_SLUG || 'thatmusicteacher';
  ...
  return `
    <a class="tc-link-btn" href="${escapeHtml(href)}" target="_blank" rel="noopener noreferrer"
       data-copy-email="${emailAttr}" onclick="window.__tcCopyEmail && window.__tcCopyEmail(this)">Learn</a>
    <script>
      window.__tcCopyEmail = function (btn) {
