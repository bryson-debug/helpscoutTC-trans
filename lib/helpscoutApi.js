...
async function getAccessToken() {
  ...
  let response;
  try {
    response = await fetchWithTimeout('https://api.helpscout.net/v2/oauth2/token', {
      method: 'POST',
...
async function getCustomerEmail(customerId) {
  ...
  let response;
  try {
    response = await fetchWithTimeout(`https://api.helpscout.net/v2/customers/${encodeURIComponent(customerId)}`, {
