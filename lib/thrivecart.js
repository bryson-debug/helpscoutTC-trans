...
// lifetime_value is a currency-keyed object, e.g. {"USD": 0} -- assumed to
// be in cents like every other amount field, though only a zero value has
// been observed live so this specific assumption is unconfirmed.
function normalizeLifetimeValue(raw) {
  if (!raw || typeof raw !== 'object') return [];
  return Object.entries(raw).map(([currency, cents]) => ({
    currency,
...
async function getCustomerTransactions(email) {
  if (isMockMode()) {
    ...
    return {
      ...
      lifetimeValue: normalizeLifetimeValue({ USD: 41500 }),
    };
  }
  ...
  return {
    ...
    purchases,
    subscriptions,
    lifetimeValue: normalizeLifetimeValue(data.lifetime_value),
  };
}
